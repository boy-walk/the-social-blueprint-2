	...
	protected function get_filter_all_price( $min_price, $max_price, $step_count, $default_step ) {
		...
		$sql = "
			...
			WHERE product.min_price = product.max_price AND 
				( post.post_parent = 0 OR post.post_parent IN ( SELECT object_id FROM {$wpdb->term_relationships} WHERE term_taxonomy_id = %d ) ) AND
		...
		$transient_name    = $this->get_transient_name( $sql );
