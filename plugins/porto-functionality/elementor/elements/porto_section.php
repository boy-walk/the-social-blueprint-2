		...
		if ( 'banner' == $settings['as_param'] ) {
			if ( ! empty( $settings['banner_image'] ) && ( ! empty( $settings['banner_image']['id'] ) || ! empty( $settings['banner_image']['url'] ) ) ) {
				...
				if ( is_array( $img_src ) ) {
					...
					if ( ! empty( $settings['banner_image']['id'] ) ) {
						...
						$before_html .= '<img src="' . esc_url( $img_src[0] ) . '" ' . ( ! empty( $img_src[1] ) ? 'alt="' . $img_src[1] . '" ' : '' )  . $attr_str_escaped . '>';
					}
				}
			}
		} elseif ( 'creative' == $settings['as_param'] ) {
			...
			if ( 'yes' == $settings['use_preset'] ) {
				...
				if ( 'boxed' == $settings['layout'] && ! $legacy_enabled && ! empty( $porto_settings ) ) {
					$spacing_value = '';
					if ( isset( $settings['spacing'], $settings['spacing']['size'] ) ) {
						$spacing_value = $settings['spacing']['size'];
					}
					echo '.elementor-element.elementor-element-' . $this->get_id() . ' .grid-creative { max-width: ' . ( (int) $porto_settings['container-width'] - (int) $porto_settings['grid-gutter-width'] + (int) $spacing_value ) . 'px }';
					echo '@media (min-width: 992px) and (max-width: ' . ( (int) $porto_settings['container-width'] + $porto_settings['grid-gutter-width'] - 1 ) . 'px) {';
					echo '.elementor-element.elementor-element-' . $this->get_id() . ' .grid-creative { max-width: ' . ( 960 - (int) $porto_settings['grid-gutter-width'] + (int) $spacing_value ) . 'px }';
					echo '}';
				}
				porto_creative_grid_style( $grid_layout, $grid_height_number, '.elementor-element.elementor-element-' . $this->get_id(), '' === $spacing_value ? false : intval( $spacing_value ), false, $unit, '.porto-grid-item' );
				echo '</style>';
			} else {
				global $porto_settings;
				if ( 'boxed' == $settings['layout'] && ! $legacy_enabled && ! empty( $porto_settings ) && isset( $settings['spacing1'], $settings['spacing1']['size'] ) && ( 0 === $settings['spacing1']['size'] || $settings['spacing1']['size'] ) ) {
					echo '<style scope="scope" data-id="' . esc_attr( $this->get_id() ) . '">';
					echo '@media (min-width: 992px) and (max-width: ' . ( (int) $porto_settings['container-width'] + $porto_settings['grid-gutter-width'] - 1 ) . 'px) {';
...
function porto_elementor_section_custom_control( $self, $args ) {
	...
	$self->start_controls_section(
		...
		$self->add_control(
			...
			array(
				'type'          => Controls_Manager::SELECT,
				'label'         => esc_html__( 'Content Stretch with CSS', 'porto-functionality' ),
				'description'   => esc_html__( 'Enable this option instead of native Elementor\'s one to stretch section with CSS.', 'porto-functionality' ),
				'options'       => array(
					''                        => esc_html__( 'Default', 'porto-functionality' ),
					'stretch_row_css'         => esc_html__( 'Stretch Section', 'porto-functionality' ),
					'stretch_row_content_css' => esc_html__( 'Stretch Section and Content', 'porto-functionality' ),
				),
				'label_block'   => true,
				'hide_in_inner' => true,
			)
		...
		$self->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'#header.sticky-header .elementor-element-{{ID}}.header-main.sticky' => 'padding-top: {{SIZE}}{{UNIT}} !important; padding-bottom: {{SIZE}}{{UNIT}} !important;',
				),
				'condition'  => array(
					'is_main_header' => 'yes',
				),
	...
	$self->add_responsive_control(
		...
		array(
			...
			'selectors'  => array(
				'.elementor-element-{{ID}}>.owl-carousel>.owl-dots .owl-dot span' => 'width: {{SIZE}}{{UNIT}};height: {{SIZE}}{{UNIT}};',
				'.elementor-element-{{ID}}>.dots-style-1>.owl-dots .owl-dot span::after' => 'border-width: calc( 2px + ( {{SIZE}}{{UNIT}} - 14px ) / 2 ) !important;',
			),
			'condition'  => array(
				'as_param'  => 'carousel',
				'show_dots' => 'yes',
	...
	$self->add_responsive_control(
		...
		array(
			...
			'selectors'   => array(
				'.elementor-element-{{ID}}>.owl-carousel>.owl-dots:not(.disabled)' => 'display:{{VALUE}} !important;',
			),
		)
	);
...
function porto_elementor_print_section_template( $content, $self ) {
	...
	<#
		...
		if ( settings.full_width_css ) {
			extra_attr += ' data-stretch-cls='+ settings.full_width_css;
		}
