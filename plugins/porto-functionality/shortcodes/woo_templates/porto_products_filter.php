	...
	if ( 'category' == $area ) {
		...
		if ( empty( $steps ) ) {
			...
			if ( class_exists( 'WOOCS' ) ) {
				global $WOOCS;
				$min_price = (float) $WOOCS->woocs_exchange_value( $prices->min_price );
				$max_price = (float) $WOOCS->woocs_exchange_value( $prices->max_price );
			} else {
			...
			if ( $max_price >= 1 ) {
				for ( $index = (int) floor( log10( $min_price < 1 ? 1 : $min_price ) ); $index <= (int) floor( log10( $max_price ) ); $index++ ) {
					$steps[] = array( 0 === $index ? 0 : pow( 10, $index ), pow( 10, $index + 1 ) );
			}
			}
		}
		$steps = apply_filters( 'porto_products_filter_price_range', $steps );

		$symbol_format = get_woocommerce_price_format();

		if ( 'list' == $display_type ) {
			echo '<ul class="porto-product-prices">';
			foreach ( $steps as $step ) {
				if ( $price_format ) {
					$format_text = str_replace( '$from', sprintf( $symbol_format, $price_symbol, $step[0] ), $price_format );
					$format_text = str_replace( '$to', ( isset( $step[1] ) ? sprintf( $symbol_format, $price_symbol, $step[1] ) : '' ), $format_text );
				} else {
					$format_text = sprintf( $symbol_format, $price_symbol, $step[0] ) . ' - ' . ( isset( $step[1] ) ? sprintf( $symbol_format, $price_symbol, $step[1] ) : '' );
				}
