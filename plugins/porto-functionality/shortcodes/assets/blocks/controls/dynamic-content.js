const PortoDynamicContentControl = function( {
	...
	let toolset_fields = [{ label: __( 'Please select...', 'porto-functionality' ), value: '' }];
	if ( porto_block_vars.toolset && porto_block_vars.toolset[options.field_type] ) {
		porto_block_vars.toolset[options.field_type].forEach( function( field_arr, index ) {
			_.forEach( field_arr.options, function( label, key ) {
				toolset_fields.push( { label: field_arr.label + ' - ' + label, value: key } );
			} );
		} );
	}
	const [toolsetFields, setToolsetFields] = useState( toolset_fields );

	useMemo(
		() => {
			toolset_fields = [{ label: __( 'Please select...', 'porto-functionality' ), value: '' }];
			if ( porto_block_vars.toolset && porto_block_vars.toolset[options.field_type] ) {
				porto_block_vars.toolset[options.field_type].forEach( function( field_arr, index ) {
					_.forEach( field_arr.options, function( label, key ) {
						toolset_fields.push( { label: field_arr.label + ' - ' + label, value: key } );
			...
			if ( toolsetFields !== toolset_fields ) {
				setToolsetFields( toolset_fields );
			}
		},
		[porto_block_vars.toolset]
	);
	...
	let source_fields = [{ label: __( 'Please select...', 'porto-functionality' ), value: '' }, { label: __( 'Page or Post Info', 'porto-functionality' ), value: 'post' }, { label: __( 'Porto Meta Box Field', 'porto-functionality' ), value: 'metabox' }, { label: __( 'Advanced Custom Field', 'porto-functionality' ), value: 'acf' }, { label: __( 'Toolset', 'porto-functionality' ), value: 'toolset' }, { label: __( 'Meta Field', 'porto-functionality' ), value: 'meta' }, { label: __( 'Taxonomy', 'porto-functionality' ), value: 'tax' },];
	...
	return el(
		...
		'toolset' == value.source && el( SelectControl, {
			label: __( 'Toolset', 'porto-functionality' ),
			value: value.toolset,
			options: toolsetFields,
			onChange: ( val ) => { value.toolset = val; onChange( value ); },
		} ),
