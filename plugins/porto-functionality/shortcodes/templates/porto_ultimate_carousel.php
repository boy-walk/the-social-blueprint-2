...
<script>
	(function(){
		var porto_inc_slick_js = function() {
			( function( $ ) {
				if ( $.fn.slick ) {
					$('.porto-carousel-<?php echo porto_filter_output( $uid_escaped ); ?>').slick({<?php echo porto_filter_output( $settings_escaped ); ?>});
				} else {
				<?php if ( wp_script_is( 'porto_shortcodes_ultimate_carousel_loader_js', 'registered' ) ) : ?>
					...
					setTimeout(function() {
						if ($.fn.slick) { $('.porto-carousel-<?php echo porto_filter_output( $uid_escaped ); ?>').slick({<?php echo porto_filter_output( $settings_escaped ); ?>}); }
					}, 300);
				<?php endif; ?>
				}
			} )( jQuery );
