			...
			searchCats.on( 'change', function ( e ) {
				...
				if ( searchCats.val() && searchCats.val() != '0' ) {
					searchForm.setOptions( {
						serviceUrl: p_ajax_url + ( -1 === p_ajax_url.indexOf( '?' ) ? '?' : '&' )  + 'action=porto_ajax_search_posts&cat=' + searchCats.val()
					} );
				} else {
					searchForm.setOptions( {
						serviceUrl: p_ajax_url + ( -1 === p_ajax_url.indexOf( '?' ) ? '?' : '&' )  + 'action=porto_ajax_search_posts'
					} );
				}
